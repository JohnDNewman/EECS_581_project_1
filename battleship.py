...
class Battleship:                                           
    ...
    def takeTurn(self, shootCoords):                    # This function is called when a player starts their turn
        if self.curBoard.shoot(shootCoords) == True:    # Pass the coordinates that were shot at to Board and check if they are valid
            if self.curBoard.allSunk() == True:         # If so, check if that shot sunk the last ship on the board
